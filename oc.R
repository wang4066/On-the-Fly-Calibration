...
mem <- function(resp,item,Q=21,mu,sig,eps =1e-3,max.iter=500)
{
  ...
  mu1 <- sum(Xg*colSums(Pjq,na.rm = T))/N
  sig1 <- sqrt(sum((Xg-mu1)^2*colSums(Pjq,na.rm = T))/N)
  #   
  # ts <- c(sig)
  ...
  while(max(df.a)>eps & max(df.b)>eps  & iter<max.iter)
  {
    ...
    GH <- normal.qu(Q,mu=mu1,sigma = sig1)
    Xg <- GH$quad.points
    Ag <- GH$quad.weights
  ...
  return(list(est=cbind(a,b),iter=iter,pop=c(mu1,sig1),ts=ts))
}
